...
`TypeScript` `Java` `Python` · `Node.js` `NestJS` `Spring Boot` `FastAPI` · `React` `Next.js 15` · `LangChain` `LangGraph` `MCP` `Pinecone` · `PostgreSQL` `MongoDB` `Redis` · `Docker` `Cloud Run` `AWS Lambda` · `Socket.IO`
